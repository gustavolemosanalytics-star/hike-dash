...
export function BudgetClient({ transactions, budgetData }: BudgetClientProps) {
    ...
    // Chart data for Recebido vs A Receber by all BUs
    const chartData = useMemo(() => {
        return combinedData.map(bu => ({
            name: bu.buName,
    ...
    // Calculate global total for the main gauge
    const globalTotal = useMemo(() => {
        return combinedData.reduce((acc, bu) => acc + bu.realizadoTotal, 0);
    }, [combinedData]);

    const globalMeta = useMemo(() => {
        return Object.values(metasByBU).reduce((a, b) => a + b, 0) || 1000000;
    }, [metasByBU]);
            ...
            <PageContent>
                ...
                <div className="glass-card rounded-2xl p-6 border border-white/40 mb-8">
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                        ...
                        {/* Right: Bar chart - Recebido vs A Receber by BU */}
                        <div>
                                ...
                                {chartData.length > 0 ? (
                                    <ResponsiveContainer width="100%" height="100%">
                                        <BarChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 5 }}>
                                            ...
                                            <Bar dataKey="Recebido" fill="#DCEEAA" radius={[4, 4, 0, 0]}>
                                                <LabelList
                                                    dataKey="Recebido"
                                                    position="top"
                                                    formatter={(val: any) => formatCompact(Number(val))}
                                                    style={{ fontSize: 9, fill: '#333', fontWeight: 600 }}
                                                />
                                            </Bar>
                                            <Bar dataKey="A Receber" fill="#4A5568" radius={[4, 4, 0, 0]}>
                                                <LabelList
                                                    dataKey="A Receber"
                                                    position="top"
                                                    formatter={(val: any) => formatCompact(Number(val))}
